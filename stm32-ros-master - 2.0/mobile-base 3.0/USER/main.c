...
u32 Distance;                               //超声波测距
u8 delay_50,delay_flag,Bi_zhang=0,PID_Send,Flash_Send=0; //延时和调参等变量
...
int main(void)
{
	...
	uart_init(57600);              //=====延时初始化
	NVIC_PriorityGroupConfig(NVIC_PriorityGroup_2);//=====设置系统中断优先级分组2
	DMA_Use_USART1_Tx_Init();				//DMA初始化
  ...
  while(1){
		data_pack();	
		if(com_x_rcv_data.linear_vx.fv==64)
			Flag_Qian=1,Flag_Hou=0,Flag_Left=0,Flag_Right=0;
		if(com_x_rcv_data.linear_vx.fv==0)
